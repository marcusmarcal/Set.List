...
// [
//   { "id": "slug", "name": "My List", "spotify_id": "4pcomesNQ...", "is_default": true },
//   ...
// ]
...
function loadPlaylists() {
    ...
    if (!file_exists($playlistsFile)) {
        // Bootstrap with the original playlist
        $default = [
            [
                'id'         => 'principal',
                'name'       => 'Marcvs Marcal',
                'spotify_id' => '4pcomesNQA6DPXj1HFpOjf',
                'is_default' => true
            ]
        ];
...
function getDefaultPlaylist($playlists) {
    foreach ($playlists as $pl) {
        if (!empty($pl['is_default'])) return $pl;
    }
...
function getActivePlaylist() {
    ...
    $id = $_GET['pl'] ?? $_SESSION['active_pl'] ?? null;
    if ($id) {
        foreach ($playlists as $pl) {
...
function songsFile($playlist) {
    // Each playlist has its own songs file: songs_<id>.json
...
function loadSongs($playlist) {
    ...
    // Fallback: if it's the default playlist and file missing, use legacy songs.json
    if (!file_exists($file)) {
...
function renderSidebar($activePage = 'index') {
    ...
    $navItems = [
        ['href' => 'index.php' . $plParam,      'label' => 'Músicas',         'page' => 'index',    'icon' => '<path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/>'],
        ['href' => 'add.php' . $plParam,         'label' => 'Adicionar',       'page' => 'add',      'icon' => '<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/>'],
        ['href' => 'import.php' . $plParam,      'label' => 'Importar Spotify','page' => 'import',   'icon' => '<polyline points="8 17 12 21 16 17"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.88 18.09A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.29"/>'],
        ['href' => 'playlists.php',              'label' => 'Listas Spotify',  'page' => 'playlists','icon' => '<line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/>'],
        ['href' => 'print_songs.php' . $plParam, 'label' => 'Imprimir',        'page' => 'print',    'icon' => '<polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/>'],
    ];
    ?>
    <aside class="sidebar">
      ...
      <div class="sidebar-playlists">
        <div class="sidebar-section-label" style="margin-bottom:12px">Listas</div>
        <?php foreach ($playlists as $pl): ?>
          <?php $isActive = ($pl['id'] === $activePlId); ?>
