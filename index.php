...
function needAuth() {
  if (isLocked() && !isAuthed()) {
      jsonOut(['error' => 'Não autorizado. Faça login primeiro.']);
  }
}
...
// O restante do seu arquivo (UUID, carregamento de dados, HTML) continua daqui para baixo...
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    $act = $_POST['_action'] ?? $_GET['_action'] ?? '';
    ...
    if($act==='public_events'){
        function _publicSongRow($s){
            $uri = $s['spotify_uri'] ?? '';
            $url = $s['spotify_url'] ?? '';
            if(!$uri && $url && preg_match('#track/([A-Za-z0-9]+)#',$url,$m))
                $uri = 'spotify:track:'.$m[1];
            $trackId = '';
            if(preg_match('#track[/:]([A-Za-z0-9]+)#', $uri.$url, $m)) $trackId = $m[1];
            return [
                'title'       => $s['title'] ?? '',
                'artist'      => $s['artist'] ?? '',
                'key'         => $s['key'] ?? '',
                'spotify_url' => $url ?: ($trackId ? "https://open.spotify.com/track/$trackId" : ''),
                'spotify_uri' => $uri,
            ];
        }
        $db   = loadDb();
        ...
        foreach($tags as $tid => $t){
            ...
            $order = !empty($t['song_order']) ? $t['song_order'] : null;
            $list  = [];
            if($order){
                foreach($order as $sid){
                    $s = $songs[$sid] ?? null;
                    if(!$s || !in_array($tid, $s['tags']??[])) continue;
                    $list[] = _publicSongRow($s);
                }
            } else {
                foreach($songs as $s){
                    if(!in_array($tid, $s['tags']??[])) continue;
                    $list[] = _publicSongRow($s);
                }
            }
            $out[] = [
                ...
                'spotify_playlist'=> $t['spotify_id'] ? "https://open.spotify.com/playlist/{$t['spotify_id']}" : '',
                'songs'         => $list,
            ];
        }
    ...
    if($act==='export_tag_to_spotify'){
        ...
        $desc = 'Exportado por SetList em '.date('d/m/Y');
        $name = $tag['name'];
        if($playlistId){
            ...
            if($code>=400) jsonOut(['ok'=>false,'error'=>"Erro ao actualizar playlist ($code)."]);
    ...
    if($act==='import_plain_text'){
        needAuth();
        $db      = loadDb();
        $text    = trim($_POST['plain_text'] ?? '');
        $tagId   = trim($_POST['tag_id'] ?? '');
        $tagName = trim($_POST['tag_new'] ?? '');
        if(!$text) jsonOut(['ok'=>false,'error'=>'Nenhum texto recebido.']);
        // Resolve/cria tag
        if(!$tagId && $tagName){
            $tagId = 'tag_'.substr(md5($tagName.time()),0,8);
            $db['tags'][$tagId] = ['id'=>$tagId,'name'=>$tagName,'type'=>'list','color'=>'','archived'=>false,'song_order'=>[]];
        } elseif($tagId && !isset($db['tags'][$tagId])){
            jsonOut(['ok'=>false,'error'=>'Tag não encontrada.']);
        }
        $added = 0; $matched = 0;
        foreach(explode("\n", $text) as $line){
            $line = trim($line);
            ...
            foreach($db['songs'] as $s){
                if(strtolower(preg_replace('/\s+/','',$s['title']??''))===$norm){ $found=$s['id']; break; }
            }
            if($found){
                $matched++;
                if($tagId && !in_array($tagId,$db['songs'][$found]['tags']??[])){
                    $db['songs'][$found]['tags'][] = $tagId;
                }
            } else {
                $uuid = 'sid_'.substr(md5($title.$artist.mt_rand()),0,8);
                $db['songs'][$uuid] = [
                    'id'=>$uuid,'title'=>$title,'artist'=>$artist,
                    'cifra_url'=>'','cifra_source'=>'','spotify_url'=>'','spotify_uri'=>'',
                    'duration_ms'=>0,'tags'=>$tagId?[$tagId]:[],'key'=>'','bpm'=>'','rhythm'=>'','capo'=>'','notes'=>'',
                ];
                $added++;
            }
        }
        if($tagId) $db['tags'][$tagId]['song_order'] = $db['tags'][$tagId]['song_order'] ?? [];
        saveDb($db);
        jsonOut(['ok'=>true,'added'=>$added,'matched'=>$matched,'tag_id'=>$tagId]);
    }
    ...
    if($act==='edit_song'){
        ...
        $s = &$db['songs'][$uuid];
        $fields = ['title','artist','cifra_url','cifra_source','spotify_url','spotify_uri','key','bpm','rhythm','capo','notes'];
...
@media (max-width: 768px) {
  ...
  /* Ocultar: Tags(5), Tom(6), Ritmo(7), BPM(8), Acções(9) */
  table thead th:nth-child(1),
  table tbody td:nth-child(1){display:none} /* drag — já oculto mas garantir */
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sb-bottom">
    ...
    <?php endif; ?>
    <?php if(isAuthed()): ?>
    <button class="sb-btn" onclick="openModal('plainTextImportModal')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="17" y1="10" x2="3" y2="10"/><line x1="21" y1="6" x2="3" y2="6"/><line x1="21" y1="14" x2="3" y2="14"/><line x1="13" y1="18" x2="3" y2="18"/></svg>
...
<main class="main">
  <div class="topbar">
    ...
    <div style="margin-left:auto;display:flex;gap:8px;align-items:center">
      <button class="btn no-print" id="exportSpotifyBtn" onclick="openExportSpotify()" style="display:none;background:#1db954;border-color:#1db954;color:#000;font-weight:600">
        <svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13"><path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm4.586 14.424a.623.623 0 01-.857.207c-2.348-1.435-5.304-1.76-8.785-.964a.623.623 0 01-.277-1.215c3.809-.87 7.077-.496 9.713 1.115a.623.623 0 01.206.857zm1.223-2.722a.779.779 0 01-1.072.257c-2.687-1.652-6.785-2.131-9.965-1.166a.779.779 0 01-.973-.52.779.779 0 01.52-.972c3.632-1.102 8.147-.568 11.233 1.329a.779.779 0 01.257 1.072zm.105-2.835C14.692 8.95 9.375 8.775 6.297 9.71a.935.623 0 11-.543-1.79c3.533-1.072 9.404-.865 13.115 1.338a.935.935 0 11-.954 1.609z"/></svg>
  ...
  <div class="content" id="mainContent">
    ...
    <div class="filters-bar no-print">
      ...
      <select class="filter-select" id="filterCapo" onchange="applyFilters()">
        <option value="">Todos os Capos</option>
        <?php for($c=1;$c<=12;$c++): ?><option value="<?=$c?>">Capo <?=$c?></option><?php endfor; ?>
      </select>
...
<div class="modal-overlay" id="addSongModal">
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="fi-row fg">
        <div style="flex:1"><label class="fl">Capo</label>
          <select class="fi" id="asCapo" onchange="updateAsCapoFigura()">
            <option value="">Sem Capo</option>
            <?php for($c=1;$c<=12;$c++): ?><option value="<?=$c?>">Capo <?=$c?></option><?php endfor; ?>
          </select>
        </div>
        <div style="flex:2;display:flex;align-items:flex-end">
          <div id="asCapoFigura" style="padding:9px 12px;font-family:'DM Mono',monospace;font-size:.8rem;color:var(--text3);background:var(--bg3);border:1px solid var(--border2);border-radius:var(--r);width:100%">—</div>
        </div>
      </div>
...
<div class="modal-overlay" id="editSongModal">
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="fi-row fg">
        <div style="flex:1"><label class="fl">Capo</label>
          <select class="fi" id="esCapo" onchange="updateEsCapoFigura()">
            <option value="">Sem Capo</option>
            <?php for($c=1;$c<=12;$c++): ?><option value="<?=$c?>">Capo <?=$c?></option><?php endfor; ?>
          </select>
        </div>
        <div style="flex:2;display:flex;flex-direction:column;justify-content:flex-end">
          <label class="fl">Figura (calculada)</label>
          <div id="esCapoFigura" style="padding:9px 12px;font-family:'DM Mono',monospace;font-size:.8rem;color:var(--accent);background:var(--bg3);border:1px solid var(--border2);border-radius:var(--r);width:100%">—</div>
        </div>
      </div>
      <div class="fg"><label class="fl">Tags</label>
...
<div class="modal-overlay" id="printModal">
  <div class="modal" style="max-width:500px">
    ...
    <div class="modal-body">
      ...
      <div class="fg">
        <label class="fl" style="display:flex;align-items:center;justify-content:space-between">
          <span>Capo a mostrar <span style="color:var(--text3);font-weight:400">(nenhum = coluna oculta)</span></span>
          <button class="btn btn-ghost" style="padding:1px 7px;font-size:.65rem" onclick="toggleAllPrintChips('printCapoChips')">Todas/Nenhuma</button>
        </label>
        <div id="printCapoChips" class="print-tag-sel-wrap"></div>
        <div id="printCapoChipsEmpty" style="font-size:.7rem;color:var(--text3);display:none;padding:3px 2px">Nenhum Capo definido nas músicas desta lista.</div>
      </div>

      <div class="fg">
        <label class="fl">Figura (acorde com Capo)</label>
        <label style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:.8rem;color:var(--text2);cursor:pointer">
          <input type="checkbox" id="printShowFigura" style="width:15px;height:15px;cursor:pointer">
          Mostrar coluna "Figura" (tom real tocado com capo)
        </label>
      </div>
    </div>
    <div class="modal-footer">
...
<div class="modal-overlay" id="plainTextImportModal">
  <div class="modal" style="max-width:520px">
    <div class="modal-title">Importar Lista de Texto</div>
    <div class="modal-body">
      <div id="plainTextImportErr"></div>
      <div class="fg">
        <label class="fl">Formato aceite</label>
        <div style="font-size:.72rem;color:var(--text3);padding:6px 10px;background:var(--bg3);border-radius:var(--r);font-family:'DM Mono',monospace;line-height:1.8">
          Título da Música - Nome do Artista<br>
          Outra Música - Outro Artista<br>
          <span style="opacity:.5">← uma por linha, artista opcional</span>
        </div>
      </div>
      <div class="fg">
        <label class="fl">Lista de músicas</label>
        <textarea class="fi" id="ptText" rows="10" placeholder="Proud Mary - Creedence Clearwater Revival&#10;Hotel California - Eagles&#10;Wonderwall - Oasis" style="font-family:'DM Mono',monospace;font-size:.76rem;resize:vertical"></textarea>
      </div>
      <div class="fg">
        <label class="fl">Adicionar a uma tag/lista existente</label>
        <select class="fi" id="ptTagId">
          <option value="">— Nenhuma (só adicionar músicas) —</option>
        </select>
      </div>
      <div class="fg">
        <label class="fl">Ou criar nova tag com o nome</label>
        <input class="fi" id="ptTagNew" placeholder="Ex: Setlist Agosto 2026">
        <div style="font-size:.7rem;color:var(--text3);margin-top:4px">Se preenchido, cria uma nova tag e associa todas as músicas.</div>
      </div>
    </div>
    <div class="modal-footer">
      <button class="btn btn-ghost" onclick="closeModal('plainTextImportModal')">Cancelar</button>
      <button class="btn btn-primary" onclick="submitPlainTextImport()">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
        Importar
      </button>
    </div>
...
function renderSidebar(){
  ...
  function tagSection(label, tagArr, dotClass, isEvent){
    ...
    const arr = isEvent ? tagArr : tagArr; // already sorted by caller
    arr.forEach(t => {
...
function applyFilters(){
  ...
  const fCapo   = document.getElementById('filterCapo').value;
  ...
  // Picker: "Sem tag" ou include/exclude
  ...
  if(fCapo)   songs = songs.filter(s=>String(s.capo||'')===fCapo);
  ...
  const hasExtraFilters = !!(includeTags.length || excludeTags.length || noTag || fRhythm || fCapo || fKey || q);
...
// ── Capo: calcula a figura do acorde ─────────────────────────────
// Ordem cromática: índice 0 = C, segue semitons ascendentes
const CHROMATIC = ['C','C#','D','D#','E','F','F#','G','G#','A','A#','B'];
const CHROMATIC_M = ['Cm','C#m','Dm','D#m','Em','Fm','F#m','Gm','G#m','Am','A#m','Bm'];

function capoFigura(key, capo){
  if(!key || !capo) return '';
  const c = parseInt(capo);
  if(!c || c < 1 || c > 12) return '';
  const isMinor = key.endsWith('m') && key.length > 1;
  const arr = isMinor ? CHROMATIC_M : CHROMATIC;
  const idx = arr.indexOf(key);
  if(idx === -1) return '';
  // Figura = tom original transposto PARA BAIXO pelo nº de semitons do capo
  const figIdx = ((idx - c) % 12 + 12) % 12;
  return arr[figIdx];
}
...
function renderTable(songs, canOrder){
  ...
  songs.forEach((s, i) => {
    ...
    const figura   = capoFigura(s.key, s.capo);
    const capoHtml = s.capo   ? `<span class="chip chip-bpm" title="${figura?'Figura: '+figura:''}">Capo ${escH(String(s.capo))}${figura?' <span style="opacity:.6;font-size:.85em">≈ '+escH(figura)+'</span>':''}</span>` : '';
    ...
    html += `<tr class="song-row${isSel?' selected':''}" data-id="${s.id}" onclick="toggleSongSelection('${s.id}')">
      ...
      <td>${keyHtml}</td><td>${capoHtml}</td><td>${rHtml}</td><td>${bpmHtml}</td>
...
function openModal(id){
  ...
  if(id==='addSongModal'){
    renderTagsPicker('asTagsPicker',[]);
    populateRhythmSelects();
    document.getElementById('asKey').onchange = updateAsCapoFigura;
    document.getElementById('asCapo').value = '';
    document.getElementById('asCapoFigura').textContent = '—';
    document.getElementById('asCapoFigura').style.color = 'var(--text3)';
  }
  // Reset spotify import modal
  if(id==='plainTextImportModal'){
    const tags = DB.tags || {};
    let opts = '<option value="">— Nenhuma (só adicionar músicas) —</option>';
    Object.values(tags).filter(t=>!t.archived).forEach(t=>{
      opts += `<option value="${t.id}">${escH(t.name)}</option>`;
    });
    document.getElementById('ptTagId').innerHTML = opts;
    document.getElementById('ptTagNew').value = '';
    document.getElementById('ptText').value = '';
    document.getElementById('plainTextImportErr').innerHTML = '';
...
function _showCapoFigura(keyId, capoId, figId){
  const key  = document.getElementById(keyId)?.value || '';
  const capo = document.getElementById(capoId)?.value || '';
  const fig  = capoFigura(key, capo);
  const el   = document.getElementById(figId);
  if(!el) return;
  if(fig){
    el.textContent = `Figura: ${fig}`;
    el.style.color = 'var(--accent)';
  } else if(capo && !key){
    el.textContent = 'Seleciona a tonalidade para calcular a figura';
...
function updateAsCapoFigura(){ _showCapoFigura('asKey','asCapo','asCapoFigura'); }
function updateEsCapoFigura(){ _showCapoFigura('esKey','esCapo','esCapoFigura'); }
...
function submitAddSong(){
  ...
  post({
    ...
    capo: document.getElementById('asCapo').value,
...
function openEditSong(uuid){
  ...
  populateRhythmSelects();
  document.getElementById('esKey').onchange = updateEsCapoFigura;
  setTimeout(()=>{
    document.getElementById('esKey').value    = s.key||'';
    document.getElementById('esRhythm').value = s.rhythm||'';
    document.getElementById('esCapo').value   = s.capo||'';
    updateEsCapoFigura();
  },50);
...
function submitEditSong(){
  ...
  const spotifyUrl = document.getElementById('esSpotify').value;
  // Duração: Spotify tem prioridade; manual só entra se não houver spotify_url e campo preenchido
  const existingSong = DB.songs[uuid]||{};
  let durMs = existingSong.duration_ms||0;
  if(!spotifyUrl){
    const manual = parseDurationInput(document.getElementById('esDuration').value);
    if(manual) durMs = manual;
  }
  post({
    ...
    capo: document.getElementById('esCapo').value,
...
function copyListToClipboard(){
  ...
  if(noTag && !activeTagFilter){
    visible = visible.filter(s=>!(s.tags||[]).length);
  } else {
    includeTags.forEach(tid => { visible = visible.filter(s=>(s.tags||[]).includes(tid)); });
    excludeTags.forEach(tid => { visible = visible.filter(s=>!(s.tags||[]).includes(tid)); });
  }
  if(fRhythm) visible = visible.filter(s=>s.rhythm===fRhythm);
  if(fCapo)   visible = visible.filter(s=>String(s.capo||'')===fCapo);
  ...
  // Clipboard API (funciona em Android Chrome e desktop)
  if(navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(text).then(()=>{
      showToast('Lista copiada! (' + visible.length + ' músicas)');
    }).catch(()=>{
      fallbackCopy(text, visible.length);
    });
  } else {
    fallbackCopy(text, visible.length);
  }
...
function fallbackCopy(text, count){
  // Funciona em iOS Safari (requer focus + setSelectionRange em vez de select())
  ...
  if(ok){
    showToast('Lista copiada! (' + (count||'') + ' músicas)');
  } else {
...
function doExportToSpotify(){
  ...
  showSaving();
  post({ _action:'export_tag_to_spotify', tag_id:activeTagFilter, user_token:_spotifyUserToken }, function(r){
...
function submitPlainTextImport(){
  const text   = document.getElementById('ptText').value.trim();
  const tagId  = document.getElementById('ptTagId').value;
  const tagNew = document.getElementById('ptTagNew').value.trim();
  if(!text){ showErr('plainTextImportErr','Cola o texto com as músicas.'); return; }
  if(tagId && tagNew){ showErr('plainTextImportErr','Escolhe uma tag existente OU cria uma nova, não as duas.'); return; }
  showSaving();
  post({ _action:'import_plain_text', plain_text:text, tag_id:tagId, tag_new:tagNew }, function(r){
    hideSaving();
    if(r.ok){
      closeModal('plainTextImportModal');
      loadDbFromServer();
      showToast(`✓ ${r.added} música${r.added!==1?'s':''} adicionada${r.added!==1?'s':''}, ${r.matched} já existia${r.matched!==1?'m':''}.`);
    } else {
      showErr('plainTextImportErr', r.error||'Erro ao importar.');
    }
  });
}
...
function _renderPrintChips(){
  ...
  // ── Capo ──
  const capos = [...new Set(filtered.map(s=>String(s.capo||'')).filter(Boolean))].sort((a,b)=>parseInt(a)-parseInt(b));
  _fillChips('printCapoChips','printCapoChipsEmpty', capos.map(c=>({value:c, label:'Capo '+c})));

  // Reset figura checkbox state
  const figChk = document.getElementById('printShowFigura');
  if(figChk && !capos.length) figChk.checked = false;
}
...
function doPrint(){
  ...
  const selCapos     = _getCheckedValues('printCapoChips');
  const showFigura   = document.getElementById('printShowFigura')?.checked || false;
  ...
  const showCapoCol  = selCapos.length > 0;
  const showFiguraCol= showFigura && (showCapoCol || showKeyCol);
  ...
  const colCount = 3 + 1 + (showKeyCol?1:0) + (showRhythmCol?1:0) + (showBpmCol?1:0) + (showCapoCol?1:0) + (showFiguraCol?1:0) + (showTagCol?1:0);
  ...
  const metaTh =
    ...
    (showCapoCol   ? '<th style="padding:6px 8px;border-bottom:2px solid #ddd;font-size:.68rem;font-weight:600">Capo</th>' : '') +
    (showFiguraCol ? '<th style="padding:6px 8px;border-bottom:2px solid #ddd;font-size:.68rem;font-weight:600">Figura</th>' : '') +
  ...
  songs.forEach((s,i) => {
    ...
    const metaTd =
      ...
      (showCapoCol   ? `<td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:.72rem">${selCapos.includes(String(s.capo||'')) ? 'Capo '+escH(String(s.capo)) : ''}</td>` : '') +
      (showFiguraCol ? (()=>{ const fig=capoFigura(s.key,s.capo); return `<td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:.72rem;font-weight:600">${fig?escH(fig):''}</td>`; })() : '') +
...
function doLogin(){
  ...
  fetch(window.location.pathname, {
    ...
    else showErr('loginErr', d.error||'Senha incorreta.');
  }).catch(()=>{ showErr('loginErr','Erro de rede.'); });
}
...
>>>>>>> 3a1d447a6d25e0d1d76e49201a3060eac06f6557
