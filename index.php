...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($act==='edit_song'){
        ...
        if($i>=0&&$i<count($songs)&&$title&&$artist){
            ...
            // spotify_url: if key sent, update (empty string = clear/unlink)
            if(isset($_POST['spotify_url'])){
                $su2=trim($_POST['spotify_url']);
                $songs[$i]['spotify_url'] = $su2;
                // If URL cleared, also clear spotify_uri and duration_ms so sync shows as needed
                if(!$su2){
                    unset($songs[$i]['spotify_uri']);
                    unset($songs[$i]['duration_ms']);
                }
            }
            saveSongs($pl,$songs);
...
$needsSync   = $hasSpot ? count(array_filter($songs, function($s){ return empty($s['spotify_url']); })) : 0;
...
@media print{
  ...
  *{background:#fff!important;color:#111!important;box-shadow:none!important;text-shadow:none!important}
  ...
  table{width:100%;border-collapse:collapse}
  thead th{
    color:#aaa!important;font-size:.55rem;
    letter-spacing:.13em;text-transform:uppercase;
    padding:4px 10px;border-bottom:1.5px solid #111!important;font-weight:600;border-top:none!important
  }
  ...
  @page{size:A4 portrait;margin:1cm 1.4cm}

  /* zoom injected dynamically by doPrint() via <style id="_dynPrintStyle"> */
  /* CSS zoom (unlike transform) participates in layout, so page flow is correct */
  .print-scale-wrap{
  ...
  <div class="topbar">
    ...
    <div class="topbar-actions">
      ...
      <button class="btn btn-outline" id="syncBtn" title="Sincronizar com Spotify">
        ...
        <?php if($needsSync>0): ?><span class="sync-badge" id="syncBadge"><?= $needsSync ?></span><?php endif; ?>
      </button>
      <?php endif; ?>
...
function saveSong(){
  ...
  if(mode==='edit'){
    ...
    post(data,function(r){
      ...
      SONGS[idx].spotify_url=r.spotify_url||'';
      ...
      // Update Spotify icon + sync badge live
      var su=r.spotify_url||'';
      var sIcon=su?'<a href="'+su+'" target="_blank" class="spot-link"><svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14"><path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm4.586 14.424a.623.623 0 0 1-.857.207c-2.348-1.435-5.304-1.76-8.785-.964a.623.623 0 1 1-.277-1.215c3.809-.87 7.076-.496 9.712 1.115a.623.623 0 0 1 .207.857zm1.223-2.722a.78.78 0 0 1-1.072.257c-2.687-1.652-6.785-2.131-9.965-1.166a.78.78 0 1 1-.453-1.492c3.633-1.102 8.147-.568 11.233 1.329a.78.78 0 0 1 .257 1.072zm.105-2.835C14.692 8.95 9.375 8.775 6.297 9.71a.937.937 0 1 1-.543-1.793c3.521-1.068 9.376-.862 13.066 1.346a.937.937 0 0 1-.906 1.604z"/></svg></a>':'';
      row.find('.td-spot').html(sIcon);
      var missing=SONGS.filter(function(s){return !s.spotify_url;}).length;
      if(missing>0){$('#syncBadge').text(missing).show();}else{$('#syncBadge').hide();}
      row.addClass('just-edited'); setTimeout(function(){ row.removeClass('just-edited'); },1300);
      closeModal('songModal');
    });
  } else {
    var rawSpotAdd = $('#smSpotUrl').val().trim();
    if(rawSpotAdd && !rawSpotAdd.startsWith('http')){
      rawSpotAdd = 'https://open.spotify.com/track/' + rawSpotAdd.replace(/^spotify:track:/,'');
    }
    data.spotify_url = rawSpotAdd;
...
function doPrint() {
  ...
  // Use SONGS array directly — reliable, not affected by DOM visibility or modal state
  // Apply current search filter if active
  var filterVal = ($('#searchInput').val() || '').toLowerCase().trim();
  var songs = SONGS.filter(function(s){
    if (!filterVal) return true;
    return (s.title  || '').toLowerCase().indexOf(filterVal) >= 0 ||
           (s.artist || '').toLowerCase().indexOf(filterVal) >= 0;
  });
  ...
  var totalSec = songs.reduce(function(s,sg){ return s + parseDur(sg.duration); }, 0);
  var durText  = totalSec > 0 ? Math.floor(totalSec/3600)+'h '+(Math.floor(totalSec/60)%60)+'m' : '';
  ...
  songs.forEach(function(s, i){
    ...
    rowsHtml +=
      ...
      '<td class="dur">'    + esc(s.duration || '') + '</td>' +
      '</tr>';
  });

  // Calculate font/padding to fill A4 (257mm printable height minus ~40mm header = 217mm for table)
  var TABLE_MM = 217;
  var rowMM    = rowCount > 0 ? TABLE_MM / rowCount : 8;
  var fontPt   = Math.min(Math.max(rowMM * 2.2, 5), 11);
  var padPt    = Math.max((rowMM * 2.835 - fontPt * 1.2) / 2, 1);
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="utf-8">' +
    ...
    'h2{font-size:16pt;font-weight:700;margin-bottom:3pt}' +
    '.sub{font-size:7pt;color:#888;letter-spacing:.06em;text-transform:uppercase;margin-bottom:3pt}' +
    '.meta{font-size:7pt;color:#555;margin-bottom:8pt}' +
    '.stats{display:flex;gap:10pt;margin-bottom:10pt}' +
    ...
    'thead th{font-size:6pt;letter-spacing:.12em;text-transform:uppercase;color:#aaa;' +
      'padding:3pt 6pt;border-bottom:1pt solid #111;text-align:left;font-weight:600}' +
    'td{font-size:' + fontPt.toFixed(1) + 'pt;' +
      'padding:' + padPt.toFixed(1) + 'pt 6pt;' +
      'border-bottom:.4pt solid #eee;vertical-align:middle}' +
    'td.num,th.num{width:24pt;text-align:right;padding-right:8pt;font-family:monospace;color:#aaa;font-size:'+(fontPt*0.82).toFixed(1)+'pt}' +
    'td.artist{color:#555;font-size:'+(fontPt*0.9).toFixed(1)+'pt}' +
    'td.dur,th.dur{text-align:right;font-family:monospace;color:#999;white-space:nowrap;font-size:'+(fontPt*0.82).toFixed(1)+'pt}' +
    ...
    '<div class="stats">' +
      ...
      (durText ? '<div class="stat"><span class="stat-n">' + durText + '</span><span class="stat-l">Duração</span></div>' : '') +
    '</div>' +
    '<hr>' +
    '<table><thead><tr>' +
      '<th class="num">#</th><th>Título</th><th>Artista</th><th class="dur">Dur.</th>' +
